...
const obterDadosRastreamento = async (nota) => {
  try {
    ...
    const headers = {
      Authorization: `Bearer ${process.env.PRINCESA_TOKEN}`,
    };

    const apiResponse = await fetchWithRetry(url, headers);
...
const getDadosPrincesa = async (options = {}) => {
  try {
    const {
    ...
    return {
      status: "error",
      errorMessage: `Falha ao consultar serviço da Princesa: ${error.message}`,
    };
  }
};

const getDadosPrincesaPorData = async (options = {}) => {
  try {
    const {
      forcarAtualizacao = false,
      horasParaAtualizar = 4,
